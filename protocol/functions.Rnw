...
plotBenchmark <- function(..., xAxis, yAxis, splitBy, legendPos="topright", log="", aggFun=min) {
    ...
    # generate the plot
    p <- ggplot(data=ad, aes(xAxis, x, color=comment, shape=comment)) + geom_line() + geom_point()
    # axis labels
...
plotOverCpus <- function(..., yAxis, splitBy, legendPos="topright", log="", from=1, to=80, aggFun=min) 
{
    ...
    # generate the plot
    p <- ggplot(data=fd, aes(xAxis, x, color=comment, shape=comment)) + geom_line() + geom_point()
    # axis labels
    p <- p + xlab(desc[xAxis]) + ylab(desc[yAxis])
