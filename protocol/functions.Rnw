...
plotOverCpus <- function(..., yAxis, splitBy, legendPos="topright", log="", from=1, to=80) {
    d <- readDataFrom(...)
    xAxis="cpus"
    
    # take the minimum of all runs
    ad <- aggregate(d[[yAxis]], list(test = d$test, algorithm = d$algorithm, scheduler = d$scheduler, comment = d$comment, xAxis = d[[xAxis]]), min)       
