...
We will refer to the multi-dimensional priority of a task as a \emph{priority vector}, which may in general be a vector of the form described in Section \ref{sec:pareto_optima}. 
...
We will use this representation of a priority vector when referencing it in pseudo-code. 
...
\subsubsection*{Spying} \label{sec:mcpq:stealing}
% Spying
...
\subsection{High level description and interfaces} \label{sec:mcpq:interface}
...
The \verb|ParetoItem| class has to guarantee that it's \verb|take| operation returns the referenced task exactly once, even if multiple threads execute it concurrently. 
...
The \verb|dead_task| function is used by the scheduler to query whether the associated task may be dropped without being executed\footnote{This feature is called ``speculative execution'' is described in Section \ref{sec:pheet:task_priorites}}.
...
\subsection{Implementation and analysis} \label{sec:mcpq:impl_analysis}
...
Note that the virtual array may be accessed by several threads at the same time.
Since we provide access to the elements of the virtual array via iterators, it might occur that one thread wants to access an element via that iterator, while another thread already decreased the capacity of the virtual array s.t.\ the element is not in the valid range anymore\todo{there should be a footnote to the section that makes it clear why this can happen, i.e., ParetoTaskStoragePlace}. 
To avoid accessing an invalid memory location, the operation \verb|decrease_capacity| merely marks an amount of items equal to \verb|value| as reusable.
That is, given an initial capacity $c$, the range $[c - \verb|value|, c]$ may still be accessed after the operation returns. 
