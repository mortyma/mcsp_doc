...
\todo{explain a bit about StrategyScheduling?}
...
\subsubsection{Strategy 2 scheduler} \label{sec:pheet:strategy2sched}

\subsubsection{Dead tasks} \label{sec:pheet:dead_tasks}

\subsection{lsm, klsm and strategy scheduling}
