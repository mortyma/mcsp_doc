...
Such a relation $\tor$ has to satisfy the following properties\footnote{We denote the concatenation of two paths $p_1 \in P_{u,v}$, $p_2 \in P_{v,w}$ as $p_1 \cup p_2$. }:
\begin{align}
\forall p,q \in P_{u,v}: p \prec q \implies p \tor q  \quad \text{Dominance} \\
\forall p \in P_{u,v}, \forall (v,w) \in A: p \tor p \cup \langle w \rangle \quad \text{Monotonic}
\end{align}
...
In this section, we provide an introduction to the concepts used in Pheet that are relevant for this technical report. While Sections \ref{sec:pheet:task_parallel}, \ref{sec:pheet:scheduling} and \ref{sec:pheet:work_stealing} provide a high-level overview of general concepts within the context of Pheet, Sections \ref{sec:pheet:places}, \ref{sec:pheet:task_priorites} and \ref{sec:pheet:priority_sched} give an introduction to Pheet-specific concepts. 
...
Pheet is a task-parallel programming library built on the well known \emph{work stealing pattern} with the goal of providing a simple-to-use framework for the quick parallelization of algorithms.
...
\subsubsection{Task scheduling} \label{sec:pheet:scheduling}
...
\subsubsection{Work stealing} \label{sec:pheet:work_stealing}

\subsubsection{Task priorities} \label{sec:pheet:task_priorites}

\subsubsection{Priority scheduling} \label{sec:pheet:priority_sched}
``Priority scheduling differs from normal scheduling due to the fact that the order in which tasks are executed is decoupled from the order in which they are created.'' \cite{Wimmer14}, page 40.
...
\todo{explain C++ memory consistency model: required for implementation of Pareto-queue}
