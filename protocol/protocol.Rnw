...
% Thread-local part
The thread-local part, which we call the \emph{Pareto task storage}, is based on the ideas of \emph{log-structured merge-lists\footnote{This idea is based on log-structured merge trees (\cite{Wimmer14}, Section 5.7).}} and \emph{partitioning}, discussed in the remained of this section. On overview of the structure of the MCPQ is depicted in Fig.\ \ref{fig:structure_overview}.
\begin{figure}
\begin{center}
\label{fig:structure_overview}
\caption{Overview of the MCPQ structure}
\begin{tikzpicture}
\end{tikzpicture}
\end{center}
\end{figure}
...
This merging operation is then continued recursively: If block $b_{i+1}$ has the same level as block $b_i$, they are merged to a block of the next higher level.
A more detailed presentation of how blocks are managed is given in Section \ref{sec:mcpq:description:managing_blocks}.
...
Furthermore, \verb|peek| does not remove the task from the priority queue.
...
\subsubsection{Peek} \label{sec:mcpq:description:pop}
% peek
Once a block is partitioned, the \verb|peek| operation is trivial.
We simply need to scan the right partition until an active item is found. 
\todo{Make partitions more clear; give them good names}
In case no active item is found in the right partition, we simply drop back to the previous partition.
Note that this partition might have to be partitioned further; once the top partition again falls below the maximum size $s_b$, we can scan for an active task.
% dead block
In case no active item can be found in the block, the whole block is marked as non-active, implying that no more peek operations have to be executed on this block.
...
While the \verb|peek| operation finds a Pareto-optimal item without removing it from the queue, the \verb|take| operation removes a given item from the mcpq by marking it as taken before returning the task contained in the item.
...
This range is identified by the offset $O$ of the block and its capacity, as shown in Fig. \ref{fig:block_offset}.
\begin{figure}
\begin{center}
\label{fig:block_offset}
\caption{A block provides access to and manages a range of tasks stored in the virtual array.}
\begin{tikzpicture}
\end{tikzpicture}
\end{center}
\end{figure}
