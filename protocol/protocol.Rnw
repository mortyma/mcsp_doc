...
\todo{\cite{Sonnier06} explains the notion of supported solutions quite well}
...
Since this number is exponential in $n$ in the worst case, the MSP is in general NP-hard.
...
We note that in contrast to the algorithms mentioned in Section \ref{sec:related}, our algorithm works on multigraphs as well and provide a comparative performance analysis in Section \todo{well, where?}.
...
\begin{center}
    ...
    Number of cores& (8 nodes with 10 cores each) \\ \hline
...
Sonnier \cite{Sonnier06} were \todo{only one author. Use plural nonetheless?} one of the first to publish parallel algorithms for solving MSP problems with 3 or 4 objectives. 
