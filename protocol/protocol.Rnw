...
Section \ref{sec:pareto} gives a formal definition of Pareto-optimality, followed by a detailed presentation and analysis of our multi-criteria priority queue (Section \ref{sec:mcpq}).
...
Each processor utilized by Pheet is assigned exactly one place, implying that a processor is uniquely identified by a place, which may be very useful for the implementation of parallel algorithms.
...
\section{Pareto optima} \label{sec:pareto}
...
Let $U_1, U_2,\dots , U_d$ be totally ordered sets (e.g., $\mathbb{N}$) and let $V$ be a set of $d$-dimensional vectors in $U_1 \times U_2 \times \dots \times U_d$.
...
\begin{align}
&\forall i \in \{1,\dots, d\}: w_i(x) \leq w_i(y) \quad \text{and}\\
&\exists j \in \{1,\dots, d\}: w_j(x) < w_j(y) \quad .
\end{align}
