...
\noindent
% data structure
A priority queue is an abstract data structure that stores a set of keys and provides operations for
inserting a key (usually called \texttt{push}) as well as finding and deleting the minimal (or highest-priority) key (\texttt{pop}), both of which are feasible in sub-linear time.
% applications
Priority queues are crucial components for numerous applications which require efficient access to the item with the minimal key, e.g., the classical single-source shortest path algorithm by Dijkstra or 
schedulers for multi-processor systems.

% parallel, concurrent priority queue
\emph{Concurrent} priority queues allow multiple processes to simultaneously access the data structure. 
They are hard to implement efficiently since the highest-priority key in the queue represents a contention point.

% whats new
While the keys of a priority queue are typically scalar values, we present a new concurrent priority queue, called the \emph{Pareto priority queue (PPQ)}, which manages keys of a more general form. In particular, keys are multi-dimensional values, i.e., vectors from some domain $V$.
Under certain assumptions, both the \texttt{push} and \texttt{pop} operation have amortized time complexity $O(\log^2 n)$. 
...
We provide an example implementation of the multi-criteria shortest path problem based on Pheet to evaluate the PPQ's performance compared to some of Pheet's existing task schedulers. 
...
and is based on the task-parallel programming model, which allows a programmer to explicitly expose the parallelism of an application to the runtime system.
...
The task scheduler utilizes a concurrent priority queue which fulfills the ordering requirements imposed by the algorithm by assigning a comparison-based priority value to each task.
...
Their claim is backed up by a parallel implementation of Dijkstra's algorithm with the help of a scheduler based on the priority task scheduling model.
...
A different advantage of using multi-dimensional priorities for task scheduling, which is independent of the algorithm that utilizes the scheduler, is that multi-dimensional priorities establish only a partial ordering on the tasks, thus giving the scheduler more flexibility in terms of which task to execute next. 
...
An intuitive solution for the MSP is to apply the $L^1$ norm to reduce the multi-dimensional priorities to scalar values, i.e., by defining a total ordering on the priority vector's domain.   
...
Computing the Pareto-optima of a set of vectors (as discussed in \cite{KungLP75}) is a well understood problem that can be solved in linear expected time \cite{BentleyCL93}. 
...
Their algorithms are based on a weighted sum approach and thus provide only an approximate solution where the solution set may not contain all Pareto-optimal paths.
% Sanders, Erb: first parallel MSP algo, but mostly for d=2
As far as we are aware, Sanders and Mandow \cite{SM13} published the first proposal for a parallel algorithm that provides an exact solution of the MSP.
...
Wimmer et al.\ \cite{WVTCT13, WimmerKLSM} present several different priority queues for task-based priority scheduling and our work is based in parts on their $k$-LSM priority queue \cite{Wimmer14}.


\section{Pareto optima} \label{sec:pareto_optima}
% Informally describe the problem
Given a set of vectors, the \emph{minimal vector problem} is to find the subset of vectors s.t.\ each vector of the subset is not dominated by any vector of the set. 
...
Such a minimal element is called \emph{Pareto-optimal} and the set of minima is called the \emph{Pareto-set}. 
% Example
...
\addplot[mark=*, only marks, point meta=explicit symbolic, color=blue]
  table {
x	y
1	7.8
1.5	7.2
...
Pheet is a task-parallel programming library with the goal of providing a sophisticated framework for the implementation and evaluation of concurrent data structures as well as different scheduler components. 
...
Furthermore, a place allows to implement processor-local data structures, e.g., parts of a distributed priority queue or application specific data structures and data which are accessible by the tasks that are executed on the associated processor.
...
The algorithm requires a priority queue (that may be implemented as e.g., a simple binary search tree or a Fibonacci heap), which is a specialized work pool.
Note that in a task parallel program, the work pool is not necessarily a centralized data structure.
...
% Task priorities 
Wimmer \cite{Wimmer14} introduced the concept of \emph{task priorities} to make the scheduler aware of the preferred (relative) execution order of tasks. 
% Discrete priorities
...
This mechanism is useful for speculative execution: A task may be spawned when it is likely that the work of the task will have to be performed sometime in the future.
When further calculations determine that the task need not be executed after all, the task can be marked dead via its associated strategy, thus instructing the scheduler not to execute the task.
...
% abstract definition
The PPQ is an \emph{ordered container}, a bag-like data structure which orders tasks by their priorities.
% keys
As usual in the literature, we will denote the items managed by a priority queue as \emph{keys}.
In the context of this work, a key is a priority vector associated with a task and a \emph{minimal} key is one with Pareto-optimal priority.
...
\begin{itemize}
\item $k_1 \less_i k_2$: The priority of $k_1$ in dimension $i$ is higher than that of $k_2$. Note that the smaller the value of $k_i$, the higher it's priority.
...
\end{itemize}
For the MSP, partial solutions consists of a path $p = \langle s,\dots,n \rangle$ form start node $s$ to node $n$ and the sum of the weight vectors of all arcs on this path, called the \emph{weight} of the path or solution.
...
A trivial solution of the problem is to compute the whole set of Pareto-optimal keys for every \texttt{pop} operation, which can be done in linear expected-time \cite{BentleyCL93}.
...
The PPQ thus employs different mechanisms, which are discussed in detail in this work.

\subsubsection*{Ordering requirements} \label{sec:ppq:ordering}
% ordering semantics  
The semantic definition of the \texttt{pop} operation requires that \emph{all} threads agree on the set of Pareto-optimal keys, implying that all \texttt{push} and \texttt{pop} operations executed by different threads need to appear to take effect in the same order for each thread, i.e., they need to be linearized w.r.t.\ all the threads.
While the existence of several Pareto optima reduces the contention in principle since different threads are allowed to pick different Pareto-optimal keys, in practice it has to be ensured that the threads do not pick the same key, i.e., the threads cannot use the same deterministic procedure to pick one key from a set of Pareto-optimal keys.

While such \emph{global ordering} semantics match the intuitive expectation, they also pose a (potentially big) performance bottleneck. 
\emph{Local ordering} semantics typically occur when each thread maintains its own, thread-local Pareto priority queue (LPQ).
A key returned by \texttt{pop} is required to be Pareto-optimal only w.r.t.\ the keys managed by the thread executing the operation; the LPQ of another thread might contain a key with strictly higher priority.
% pro and cons			
...
% Spying
When a thread does not have any more keys (and thus tasks) in its LPQ, it will try to \emph{spy} some from the LPQ of another thread. 
A thread spies keys from another LPQ by copying references to them to its own.
...
The operations of the spying thread need to be linearized with the operations of all the other threads on the same LPQ.
...
Each thread maintains its own PPQ, called the local priority queue (LPQ).
The LPQ is split into a part that may be accessed by several threads concurrently and a second part -- built on top of the first -- that may be accessed by the owning thread only.
% Virtual array
...
The Virtual Array is used to hold all the keys currently managed by the LPQ and allows other threads to scan it for keys with associated tasks that are awaiting execution.
...
The Virtual Array of size $n$ is logically divided into an ordered sequence of a logarithmic number of arrays, called \texttt{blocks}.
...
\begin{itemize}
\item \texttt{peek} finds a Pareto-optimal key w.r.t.\ the keys managed by the block. Note that \texttt{peek} on a block is similar to \texttt{pop} on the whole priority queue, except that it does not remove the key from the data structure.
\item \texttt{take} removes a given key from the priority queue.
\end{itemize}
% Overview of pop
The \texttt{pop} operation performs a \texttt{peek} operation  on each block to obtain a set of $O(\log n)$ keys (one key from each block) that are Pareto-optimal w.r.t.\ to the keys in their respective blocks.
...
Wait-free memory reuse schemes have been studied extensively in \cite{Wimmer14}, where a lightweight scheme requiring $O(1)$ amortized time to find an object ready for reuse is discussed.
Note that the scheme is restricted to uniform, ABA-safe objects for which it is possible to determine if they can be  reused.

At the time of this work, this scheme was not yet fully analyzed and implemented and thus we do not concern ourselves with memory management but make the following assumptions, justified by Lemmas 4.3.2 and 4.3.3 in \cite{Wimmer14}:

\begin{assumption} \label{ass:deleteIsConstant}
The memory management imposes an overhead of $O(1)$ amortized time per managed object.
\end{assumption}
...
Nest, we will show the time complexity of the \texttt{push} and \texttt{pop} operation of the PPQ. 
Since in lock-free algorithms a thread may be delayed by other threads for an unbounded amount of time as long as other threads regularly make progress, we show the time complexity under the following assumption:
\begin{assumption} \label{ass:sequential}
We assume that no keys are spied or marked deleted by another thread between the start and the end of the execution of a \texttt{push} or \texttt{pop} operation.
\end{assumption}
...
Section \ref{sec:ppq:description:fusing_blocks} shows how the logarithmic number of blocks is maintained.
...
To ensure thread-safety, references to the keys\footnote{We call the elements of the array \emph{keys} because it is part of the PPQ and thus deals with the same objects.} in the array need to be stored as atomic pointers.
% Operations
It provides the following operations:
\begin{itemize}
\item \texttt{begin} obtains an iterator referencing the first key in the Virtual Array.
\item \texttt{end} obtains an iterator to the past-the-end key in the Virtual Array, that is, to the hypothetical key \emph{after} the last key in the array.
...
\item \texttt{advance(n)} applies the \texttt{++} (if $n$ is negative: the \texttt{---}) operator $n$ times.
\end{itemize}

% Implementation
The Virtual Array is implemented via a doubly-linked list of arrays of constant size $s_\text{v}$, called \emph{ArrayBlocks}. 
% Analysis
The operations \texttt{begin} and \texttt{end} of the Virtual Array as well as the \texttt{++} and \texttt{---} operators of an iterator trivially require $O(1)$ time, while the \texttt{advance} operation has time complexity $O(n)$.
% Access time
% Deletion
To ensure thread safety, the \texttt{delete} operation only removes whole ArrayBlocks from the Virtual Array. 
Given iterators $l$ and $r$, it removes the range $[l', r']$, where $l' = c_1 s_v$ is the minimal $l'$ s.t.\ $l \leq l' $ and $r' = c_2 s_v$ is the maximal $r'$ s.t.\ $r' \leq r $, for constants $c_1, c_2 > 0$.
Note that keys in the range $]l, l'[$ and $]r', r[$ are not removed, i.e., after the \texttt{remove} operation the range $]l, r[$ is not necessarily empty and may contain up to $2 s_v-2$ elements. 
This is because elements in the ranges $[l,l']$ and $[r',r]$ are stored by blocks that cannot be removed as long as they contain some non-deleted elements and the size of each range is at most $s_v - 1$.
It is the user's responsibility to always apply the operation with the largest possible range as removing the range $]l,r[$ cannot be done by using the \texttt{remove} operation twice with the ranges $]l, k[$, $]k-1, r[$ for some $k$, $l<k<r$, due to the semantics of the operation.
Section \ref{sec:ppq:description:partitioning} shows how this can be done.

The \texttt{delete} operation trivially requires $O(n)$ time, where $n$ is the number of elements in the range $[l,r]$.

\begin{lemma} \label{lemma:virutalArrayMemory}
To store $n$ keys, the Virtual Array allocates $O(n)$ memory.
\end{lemma}
\begin{proof}
Assuming that the \texttt{remove} operation is applied as described above, the worst case is 1 non-deleted key per ArrayBlock.
Note that an ArrayBlock cannot be deleted as long as it contains at least one non-deleted key. 
The overhead for storing one key is maximized in this case, it is $O(s_v)$.
If an ArrayBlock does not contain any non-deleted keys, the closest preceding non-deleted key is in a preceding ArrayBlock, while the closest succeeding key is in a succeeding ArrayBlock, implying that the ArrayBlock was deleted by a \texttt{remove} operation. 
Since ArrayBlocks are of constant size $s_v$, the Virtual Array is of size at most $s_v n$.
\end{proof}
...
\subfigure [Blocks $b_1$ and $b_a$ are fused and, a new, empty ArrayBlock $b_a$ is added. ] {
    \begin{tikzpicture}[>=stealth']
    \block{0}{4}{0}{\itemWidth*0};
...
\subfigure [Blocks are fused as long as there are 2 blocks of equal level.] {
    \begin{tikzpicture}[>=stealth']
...
An LSML is a doubly-linked list of a logarithmic number of sorted arrays, called \emph{blocks}.
Note that the blocks of the LSML are different from the ArrayBlocks of the Virtual Array.
A block has a capacity of $c 2^l$, where $c$ is the capacity of the smallest block and $l$, $l\geq 0$ is the level of a block.
...
% Fusing
In case block $b_l$ is of level $0$, blocks $b_l$ and $b_a$ are \emph{fused} into a single block of level 1.
To maintain the logarithmic number of blocks, this fusing operation is applied recursively, that is, two blocks of the same level $l$ are always fused into a single block of level $l+1$.
Note that in an LSML, two blocks of same level are always next to each other. 
A new, empty block $b_a$ is created after the old one was fused with another block.
Fig.\ \ref{fig:lsml_fuse} illustrates the structure of an LSML as well as the fusing operation triggered by the insertion of a new key.
...
\subfigure [\dots and fused with its successor.] {
    \begin{tikzpicture}[>=stealth']
    \blockto{0}{4}{0}{\itemWidth*0}{4};
    \foreach \i in {0,...,3}{
...
Once half of the keys of a block $b_i$ have been removed, it is shrunk to half its original capacity and its level is decremented. If necessary, the block is fused with its successor $b_{i+1}$. 
Blocks of level 0 will be deleted once empty while the block $b_a$ is never deleted or shrunk - it is kept to handle future insertions of new keys.
Note that at most one fuse operation is necessary after shrinking a block.
Fig.\ \ref{fig:lsml_remove} illustrates this process.

\begin{corollary}
Fusing two blocks requires constant time.
\end{corollary}
\begin{proof}
Since all operations are executed in-place on the Virtual Array, two blocks are fused by doubling the capacity (i.e., the range of keys maintained by the block) of the first block and increasing its level by one. 
The second block is then deleted. 
Note that the first block thus covers all keys that before were covered by both blocks.
\end{proof}
...
The pivot element $x$ is then taken to be $x = k[i]$ (Algorithm \ref{algo:pivot}). Our procedure (Algorithm \ref{algo:partition}) for in-place partitioning multi-dimensional keys is similar to the well-known partitioning algorithm for scalar values. 
In principle, one could take the whole key as the pivot element, this however might make the partitioning infeasible if the set of Pareto-optima is large.
...
We will call these two parts the left and the right partition and use a subscript to indicate which partitioning step created them: The two partitions created by the first partitioning step are called $p_{l1}$ and $p_{r1}$.
...
% figs and refs
Fig.\ \ref{fig:block_structure} depicts the structure of a block after it was partitioned (The $p_d$ section contains deleted keys and is discussed in detail later on).
...
\begin{corollary}
Generating a pivot element requires $O(n)$ time. 
\end{corollary}
\begin{proof}
...
Assuming that all the keys managed by a block are equal, partitioning would sort all the keys into the right partition -- regardless of which key is chosen as the pivot -- and then try to partition the very same elements again.
...
After partitioning a block, the size of its right partition $p_r$ is at most the constant cut-off value $s_p$. 
\end{assumption} 
...
Once the block is fused with another block, the deleted keys of both blocks are moved to the end of the generated block.
...
This can happen at most $n$ times for the whole block, since $right <= delete$ always holds, and the worst-case complexity does not change.
...
\item the level of the block succeeding $b$ is smaller than $l$ (if such a block exists), and
...
% falling back
In case no such key is found in the right partition, we fall back to the previous partition: since the right partition does not contain any more non-deleted keys, it is fused with the section holding the deleted keys; the left partition with highest index is then taken as the new right partition.
...
It will be fused with the section holding the deleted keys by the next \texttt{pop} or \texttt{push} operation that triggers a fusing operation. 
When falling back to another partition, \texttt{peek} shrinks the block to ensure that it uses $O(n)$ memory (see Lemma \ref{lemma:partitionandshrink}) but it does not fuse blocks in case the successor of block $b$ has the same level. 
It will be shown in the following how the \texttt{push} operation ensures the logarithmic number of blocks while using \texttt{peek}.
...
\subsubsection{Fusing blocks} \label{sec:ppq:description:fusing_blocks}
\begin{algorithm}
\caption{Pseudo code for fusing several blocks}
\label{algo:fuse_from}
\begin{algorithmic}[1]
\Function{fuse\_from}{Block $b$}
\State let $b_p$ be the block preceding $b$
\State $fused \gets false$
\While{$b_p$ exists and is either of same level as $b$ }
\State fuse $b_p$ and $b$
\State $b \gets b_p$
\State $b_p \gets$ predecessor of $b$
\State $fused \gets true$
\EndWhile
\State \Return{$fused $}
\EndFunction
\end{algorithmic}
...
If another block of level 0 already exists, the two blocks are fused to create a block of level 1. 
This fusing operation of two blocks is described in detail in Section \ref{sec:ppq:description:lsm}.
This operation is continued as long as there are two blocks of same level, as shown in Algorithm \ref{algo:fuse_from} (due to the structure of the LSML, two blocks of same level are always neighbors).
The resulting block $b$ will have to be partitioned before the next \texttt{peek} operation. 

\begin{corollary}
The \texttt{fuse\_from} operation has worst-case time complexity $O(\log n)$.
\end{corollary}
\begin{proof}
...
In case the block $b_a$ is full, a new block of level 0 is created and all keys from block $b_a$ are moved there. Blocks are then fused, if necessary.
...
\begin{center}
    ...
    \end{tabular}
\end{center}
\caption{Number of keys processed in a fuse operation after inserting keys $i s_p$ keys.}
\label{table:nrFuses}
\end{table*}
...
Otherwise, creating a new block and moving all keys from block $b_a$ to the new block can be done in time $O(n)$, since obtaining an iterator to the end of the block requires $O(n)$ time (the number of keys in $b_a$ is equal to the constant $s_p$ and thus copying them requires only constant time).
Since the \texttt{fuse\_from} operation requires $O(\log n$ time it follows that \texttt{partition} dominates the complexity of the operation. 

We amortize its cost over $n$ \texttt{push} operations: 
The first time block $b_a$ is full, no fuse operation is required since no other block exists. 
The second time the new block needs to be fused with the other block of level 0, prompting a \texttt{partition} operation with $2s_p$ keys.
Once $4 s_p$ keys have been inserted, one additional fuse operation of $4 s_p$ keys is required.
However, no additional fuse operation of $2 s_p$ keys is done. 
This can be seen as follows: 
Inserting $3 s_p $ keys creates one block of level 1 and one block of level 0. 
After adding $s_p$ more keys, a new level 0 block is created and, since now two blocks of same level exist, \texttt{fuse\_from} will combine them to a level 1 block. 
Now that two blocks of level 1 exists, they are combined into a level 2 block, which is then partitioned.
Table \ref{table:nrFuses} list the number of keys processed in the \texttt{fuse\_from} and \texttt{partition} operation after a given number of keys have been added.

One fuse operation is required every $2 s_p$ keys, i.e., $n/2s_p$ fuse operations are required for $n$ keys. 
This is clearly less than one fuse of $2 s_p$ keys every $2 s_p$ keys plus one fuse of $4 s_p$ keys every $4 s_p$ keys and so on. 
...
\texttt{peek} and \texttt{fuse\_from} each require $O(\log n)$ time and thus the while loop has complexity $O(\log n)$.
...
\subsection{Implementation and integration in Pheet} \label{sec:ppq:pheet_integration}
...
It offers base classes for alternative \emph{task storage} implementations, which in Pheet terminology is a priority queue managing tasks.
% The classes we us.
...
The base class \texttt{Strategy2BaseTaskStorageBaseItem} provides some functionality for memory re-usage and serves as an interface for \texttt{StrategyScheduler2}, which is oblivious of the task storage implementation.	
...
class SchedulerTask {
  ...
  /* The code in execute() will be run when the task is executed. */
  void execute();
}
\end{code}
...
class ParetoStrategy {
  ...
  /* Return false if other has higher priority; otherwise, return true. */
  bool prioritize(ParetoStrategy other);
  
  /*Return true if the associated task has been marked dead. */
  bool dead_task();
  
  /*Return true if the associated task has been marked taken. */
  bool taken(); 
}
\end{code}
...
Furthermore, the function \texttt{prioritize}, given another \texttt{ParetoStrategy} object, decides which of the two has higher priority. 
The function is used by our task storage implementation to order tasks w.r.t.\ their associated priorities.
The \texttt{dead\_task} function is used by the scheduler to query whether the associated task may be dropped without being executed, i.e., it is used to implement the ``speculative execution'' feature as described in Section \ref{sec:pheet:task_priorites}, while the \texttt{taken} function is used to check whether the task has been chosen for execution by a different thread.
...
class ParetoTaskStorage : Strategy2BaseTaskStorage {
  ...
  SchedulerTask spy(ParetoItem boundary, int place_id) {
    return places[place_id].steal(boundary);
  }
}
\end{code}

The central -- or global -- part of the PPQ is implemented in \texttt{ParetoTaskStorage} (Listing \ref{lst:ParetoTaskStorage}), which is mostly used for rerouting \texttt{pop} and \texttt{spy} operations to the place-local parts of the PPQ, called \texttt{ParetoTaskStoragePlace}, whose interface is given in (Listing \ref{lst:ParetoTaskStoragePlace}). 
% boundary
Notice that the \texttt{pop} and \texttt{spy} operations of both classes have an additional parameter called the boundary item. 
The \texttt{StrategyScheduler2} maintains its own LIFO priority queue. 
(This is necessary for some advanced features the base data structures provides. Since we do not require them, they are not introduced in this report.)
...
\begin{itemize}
\item The \texttt{pop}/\texttt{spy} operation may select other items with a \emph{higher} priority than the boundary item, thereby violating the LIFO order. Since we are using multi-dimensional priority vectors, this means that the priority associated with the selected item may not be dominated by the priority of the task referenced by the boundary item.
...
class ParetoTaskStoragePlace : Strategy2BaseTaskStoragePlace {
  ...
  /* Spy items from another place. */
  SchedulerTask spy(ParetoItem boundary, ParetoTastStoragePlace other_place);
}
\end{code}

The description given of the PPQ given in Section \ref{sec:ppq:description} can directly be translated into an implementation of \texttt{ParetoTaskStoragePlace} and we do not give the details here.
...
Problems like this can be modeled as \emph{multi-criteria shortest path} (MSP) problems, where we are interested in \emph{Pareto-optimal} solutions. 
...
We are given a weighted directed graph $G=(V,A,w)$, where $V= \{v_1, v_2, \dots, v_n\}$ is a finite set of nodes, $A \subseteq V \times V$ is a finite set of arcs (directed edges) and $w: A \rightarrow \mathbb{N}$ is the cost or weight function. 
...
Let $p = \langle v_0, a_1, v_1, a_2, \dots,a_k, v_k \rangle$ be a path from $v_0$ to $v_k$ in $G$.
We will also write $p = \langle v_0, v_1, \dots,v_k \rangle$, since an arc $a_i$ is well defined by its two adjacent nodes. 
...
p &\mapsto w(p) = \sum_{i=1}^{k}w(a_i) \quad , \neqn{sssp_weighted}
\end{align*}
i.e., the sum of the weight of all the arcs in the path $p$.

% Variations: Single source, all-pairs,... shortest path
In a \emph{single-source shortest path problem (SSSP)}, we want to compute a shortest path from a given source node $s \in V$ to each node $v \in V$.
...
% MSP
Multi-criteria shortest path problems \cite{Martins84} generalize shortest path problems w.r.t.~the weight function $w$, which is extended to $d$-dimensional vectors, i.e., $\vect{w}: A \rightarrow \mathbb{N}^d$ and
\begin{align*}
\vect{w}: P &\rightarrow \mathbb{N}^d \\
p &\mapsto \vect{w}(p) = (w_1(p), w_2(p), \dots , w_d(p)) \quad , \neqn{msp_weighted}
\end{align*}
where $w_j(p) = \sum_{i=1}^{k}w_j(a_i)$ $\forall j \in \{1,\dots,d\}$.
...
However, it was observed that the problem is tractable for many practically relevant instances. 
...
Such a relation $\tor$ has to satisfy the following properties (We denote the concatenation of two paths $p_1 \in P_{u,v}$, $p_2 \in P_{v,w}$ as $\cP{p_1}{p_2}$):
\begin{align}
...
We provide two strategies that are based on the generic \texttt{ParetoStrategy} class (Lst.\ \ref{lst:ParetoStrategy}).
\begin{itemize}
\item The \texttt{L1Strategy} (Lst.\ \ref{lst:LinearCombinationStrategy}) assumes that a total order relation can be defined on the set of priority vectors. As Section \ref{sec:shortest_path:approx} shows, the $\leq_{sum}$ relation based on the $L¹$ norm constitutes such a relation for integer vectors (which, however, is not applicable in general).
...
Given two priority vectors, it offers a function to check whether one dominates the other as well to compare them w.r.t.\ a given dimension.
\end{itemize}
...
Pheet's \texttt{LSMLocalityTaskStorage} (see \cite{Wimmer14}) is used for tasks spawned with the \texttt{LinearCombinationStrategy} strategy.
...
We compare the execution time of the Pheet-based MSP algorithm w.r.t.\ the two scheduler variants described in Section \ref{sec:msp_algo:implementation}.
Fig.\ \ref{fig:plot1} and Fig.\ \ref{fig:plot2} indicate that the problem scales reasonably for up to 80 processors when using our PPQ and it is slower than the LSM variant by roughly a factor of two.
...
We extended this concept by allowing multi-dimensional priority vectors for tasks and discussed how and under which circumstances multi-dimensional priorities may be reduced to scalar priority values, as well as the limitations of this approach. 
An efficient implementation of a concurrent priority queue that can directly work with priority vectors (called Pareto priority queue, or PPQ) is required for, e.g., the multi-criteria shortest path problem, which we used as an example application for the PPQ and as a benchmark. 
...
In our opinion, this is a reasonable trade-off for being able to directly handle general multi-dimensional priority vectors, while to the LSM relies on the $L^1$ norm and is thus restricted to less general priority vectors, which it can handle only indirectly. 

For the analysis, some assumptions had to be made which we plan to remove in future work. 
...
Furthermore, I am especially grateful to all members of the ``Parallel Computing'' research group at the Vienna University of Technology for providing such a motivating working environment.
