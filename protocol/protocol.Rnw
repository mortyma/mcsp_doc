...
For the analysis, we will limit ourselves to the sequential case, i.e., we assume that no keys are spied or marked deleted by another thread. 
...
After partitioning the whole block, its level is reduced as far as possible and, in case the section containing deleted keys is large enough, part of them are removed from the LSML via the \verb|remove| operation of the Virtual Array (Algorithm \ref{algo:partition_block}).
...
\begin{algorithm}
\caption{Pseudo-code for partitioning a block}
\label{algo:partition_block}
\begin{algorithmic}[1]
\Function{PartitionBlock}{Block $b$}
\State \Call{Partition}{$b$, $0$, size of $b - 1$} \Comment{Step 1: Partition the block}
\State $l \gets $ level of $b$ \Comment{Step 2: Reduce the level of the block as far as possible}
...
\begin{lemma}
The time complexity of the \textsc{PartitionBlock} function (Algorithm \ref{algo:partition_block}) is dominated by the \textsc{Partition} function.
\end{lemma}
\begin{proof}
The loop at lines 5-7 is executed at most $l$ times. Since $n = 2^{\log l}$ (by definition of a block), this is equivalent to $\log n$. 
Removing a range of items from the Virtual Array requires $O(1)$ time (see Section \ref{sec:virtual_array}).
\end{proof}

\begin{lemma}
After partitioning a block, it uses $O(n)$ memory to store $n$ keys. 
\end{lemma}
\begin{proof}
Since \textsc{Partition} moves all deleted keys to the section of deleted keys, this follows directly from the call of the Virtual Array's \textsc{Remove} operation at the end of the \textsc{PartitionBlock} procedure.
\end{proof}
...
\begin{tikzpicture}
\end{tikzpicture}
\caption{Example of a partition run with deleted keys}
...
\ForAll{Keys $x$ in $p_r$} \Comment{Scan the right partition}
  ...
  \EndIf
\EndFor
\If{$k == null$} \Comment{No active task in the right partition}
  \State Extend $p_{d}$ to include $p_r$ \Comment{Add all the tasks in $p_r$ to the non-active tasks section}
  \If{$p_{l1} < p_{d}$}
    \State Set $p_r \gets p_{li}$
    \State \Call{Partition}{$b$, $p_r$, $p_{d}$} \Comment{Further partition $p_r$}
    \State $k \gets $ \Call{peek}{$b$}	\Comment{Call peek on the new right partition again}
  \EndIf
\EndIf
\State \Return{$k$}
\EndFunction
...
% falling back
In case no such key is found in the right partition, we simply fall back to the previous partition: since the right partition does not contain any key, it is merged with the section holding the deleted keys; the left partition with highest index is then taken as the new right partition.
...
% dead block
In case no such key can be found in the whole block (i.e., after falling back to the last existing partition), the whole block is marked as deleted, implying that no more peek operations have to be executed on it.

\begin{lemma}
The \verb|peek| operation has amortized time complexity $O(\log n)$.
\end{lemma}
\begin{proof}
The for-loop (lines 3-6) has constant complexity because, due to Ass.\ \ref{ass:partition_succeeds}, the size of the right partition $p_r$ is bounded by the constant $s_p$.
...
This merging operation is continued as long as there are two blocks of same level (note that due to the structure of the LSML, two blocks of same level are always neighbors).
The block resulting from this merging operation has to be partitioned again to ensure that \verb|peek| can find a Pareto-optimal key in constant amortized time \todo{this has yet to be shown}.
...
\While{$b_p$ exists and is either a deleted block or of same level as $b$ }
...
\begin{lemma}
The \verb|push| operation has amortized time complexity $O()$ \todo{well, what?}.
\end{lemma}
\begin{proof}
...
\State Block $b \gets b_a $d
\While{$b.next$ exists}
  \State $b \gets b.next$
  \If{$b$ is marked deleted}
    \State continue
  \EndIf
  \State Key $k \gets$ \Call{peek}{$b$}
  \If{$k == null$}
    \State mark $b$ as deleted block
  \Else
...
\begin{lemma}
The \verb|pop| operation has amortized time complexity $O()$ \todo{well, what?}.
\end{lemma}
\begin{proof}
...
\begin{lemma}
The \verb|spy| operation has amortized time complexity $O()$ \todo{well, what?}.
\end{lemma}
\begin{proof}
...
\begin{itemize}
\item The \verb|LinearCombinationStrategy| (Lst.\ \ref{lst:LinearCombinationStrategy} assumes that a total order relation can be defined on the set of priority vectors. As Section \ref{sec:shortest_path:approx} shows, the $\leq_{sum}$ relation based on the $L¹$ norm constitutes such a relation for integer vectors (which, however, is not applicable in general).
