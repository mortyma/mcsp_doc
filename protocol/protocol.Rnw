...
In the context of this work, an item is is a priority vector which is associated with a task.
...
\end{lemma}
\begin{proof}
Since \textsc{Partition} moves all deleted keys to the section of deleted keys, this follows directly from the call of to \textsc{Remove} at the end of the \textsc{PartitionBlock} procedure.
\end{proof}
...
\begin{lemma}
The \verb|peek| operation has amortized time complexity $O()$ \todo{what?}
\end{lemma}
\begin{proof}
