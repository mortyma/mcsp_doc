...
Let $U_1, U_2,\dots , U_d$ be totally ordered sets with a binary comparison function $<_i$ (e.g., $\mathbb{N}$ and the ``smaller than'' function) and let $V$ be a set of $d$-dimensional vectors in $U_1 \times U_2 \times \dots \times U_d$.
...
\begin{align}
&\forall i \in \{1,\dots, d\}: w_i(x) \leq_i w_i(y) \quad \text{and}\\
&\exists j \in \{1,\dots, d\}: w_j(x) <_i w_j(y) \quad .
\end{align}
Note that $\vect{x} \prec \vect{y} $ implies $\vect{x} \neq \vect{y}$.
For $d\geq 2$, the dominance relation ($\prec$) defines a partial order on $V$, but not a total order.
...
\begin{align}
\nexists \vect{v} \in V: \vect{v} \prec \vect{x} \quad ,
\end{align}
...
We simply speak of a non-dominated or Pareto-optimal task when referring to task with a non-dominated priority vector and map the mathematical concept to an object as shown in Listing \ref{lst:priority_vector}, with \verb|PriorityValue| representing a value from a domain $U_i$ for $1 \leq i \leq d$.
...
% Spying
When a thread does not have any more ready-tasks in its local priority queue, it will try to \emph{steal} some from the priority queue of another thread. 
The operations of the stealing thread are linearized with the ones of all the other threads on the same priority queue.

\subsection{Description and analysis} \label{sec:mcpq:description}
% Purely local ordering semantics
Our MCPQ observes only local ordering semantics; thus, each thread may maintain its own priority queue, which however has to allow other threads to spy on it. \todo{May be covered somewhere else}
...
% Virtual array
For the concurrent part , we assume a (potentially) infinite, thread-safe array-like data structure with linear (w.r.t.\ the capacity $n$ \todo{$n$ because that'*S the number of items} of data structure) access time for a whole sequence of $m\leq n$ items, henceforth called the \emph{Virtual Array}.
That is, sequentially accessing $m$ items that are stored in a consecutive sequence takes time $O(n)$.
(\todo{Refer to section where this is shown})
...
% Thread-local part
The thread-local part, which we call the \emph{Pareto task storage}, is based on the ideas of \emph{log-structured merge-lists\footnote{This idea is based on log-structured merge trees (\cite{Wimmer14}, Section 5.7).}} and \emph{partitioning \todo{better name?}}, explained in the following and depicted in Fig.\ \todo{A nice figure to illustrate those things}.
...
This merging operation is illustrated in Fig.\ \todo{well, need to draw a pretty picture here}.
...
%Idea
A block partitions the tasks it holds by a pivot value, which is found by randomly\footnote{All random selections in this work are meant to be made \emph{uniformly and independently.}} selecting a task with priority vector $p$ within the range to be partitioned and selecting a random value $k$ in the range $[1,\dots,d]$, where $d$ is the cardinality of the priority vector\todo{explain why we don't just take a whole vector}. 
The pivot element $x$ is then taken to be $x = w_k(p)$ and the procedure for in-place partitioning is given in Algorithm \ref{algo:partition}.
...
\While{$left < right$}
  \Comment
  \While{$left < right$ and (task $t = b[left]$, $w_k(t.p) \lessdot x$)}
  \EndWhile
\EndWhile
\EndFunction
\end{algorithmic}
...
Since the dominance relation does not constitute a total order

\subsubsection{Removed, dead and taken tasks} \label{sec:mcpq:description:dead_items}
